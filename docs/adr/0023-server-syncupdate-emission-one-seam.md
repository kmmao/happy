...
- **1.g** — rename `eventRouter.emitUpdate` to `eventRouter._emit` (internal); add an ESLint rule forbidding direct calls from outside `syncUpdate.ts`. After 1.g, the seam is **enforced**, not just available.
