...
- **Stop hook 推送通知** —— `scripts/claude-stop-notify/`。给每个会话结束发一条 ntfy 通知，标题带耗时，正文对自主 loop 会显示「本轮实际做了什么」的迭代摘要（而非注入的样板 prompt），是「可见」层最轻量的一环。含可移植脚本 + 一键 `install.sh`，便于在多台设备上复用。
