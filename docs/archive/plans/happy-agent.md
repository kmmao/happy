...
- **Server API**: REST endpoints at `https://s.sangreal.code.xycloud.info:2443` + Socket.IO at `/v1/updates`
...
- [x] Create `src/config.ts` — reads `HAPPY_SERVER_URL` (default: `https://s.sangreal.code.xycloud.info:2443`), `HAPPY_HOME_DIR` (default: `~/.happy`), derives credential file path as `${happyHomeDir}/agent.key`
