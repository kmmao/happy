...
当前 `extra.app` 中已包含 ElevenLabs 配置：
...
```bash
eas secret:create --name EXPO_PUBLIC_ELEVENLABS_AGENT_ID --value agent_xxxxxxxxxxxx
```

**方式三：Shell 环境变量（临时测试）**

```bash
EXPO_PUBLIC_ELEVENLABS_AGENT_ID=agent_xxxxxxxxxxxx yarn workspace happy-app start
```
