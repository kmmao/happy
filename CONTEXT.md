...
**SessionController**:
The App-side seam that bundles every Session-scoped read+write face — pending queue, AutoOptionSend state, Session preferences (permissionMode / autoCompact / modelMode / viewed) — behind one injection-friendly factory. Constructed via `createSessionController({ deps: { dispatcher, autoOption, storage: SessionStorageOps, interruptSession, trackMessageSent }, sessionId })` — **curried by sessionId**: each call returns a controller whose methods close over the sessionId (zero-arg atoms). Consumed in components through a hybrid hook layer (bottom: `useSessionPendingQueue` / `useSessionAutoOptionState` (read-only — state/enabled/generatedOptions/semanticScores/toggle) / `useSessionAutoOptionActions` (write-only — dispatch/recordManualSend/recordDismiss/triggerScoring*/triggerGeneration*/updateUIContext) / `useSessionPrefs` for subscription-grained reads; top: `useSessionController(sessionId)` exposing ONLY cross-domain atoms — never a data aggregator). `storage` is injected as the narrow `SessionStorageOps` interface (subscribe(cb)/getQueue(sid)/getPaused(sid)/getSession(sid) + the ~10 writes), not the full Zustand singleton — production wires `adaptStorage(storage)`, tests wire in-memory fakes. Bottom-hook reads integrate via `useSyncExternalStore(ops.subscribe, () => ops.getX(sid))` — same pattern as `appLockState.ts:48`. Cross-domain atom catalog (closed set, derived from current call sites — NEW atoms require updating this list): `interruptAndDispatch` (micro: sessionInterrupt + dispatcher.schedule), `sendHeadNow` (alias), `sendItemNow(localId)` (reorder + interruptAndDispatch), `editSaveAndSendImmediately(localId, msg, displayText)` (update + reorder + interruptAndDispatch), `sendOptionFromPopover(option, optionsHash)` (autoOption.record + autoOption.dispatch + queue.append), `sendComposerMessage({finalMessage, displayText, selectedOption?, chainAfter?})` (conditional autoOption.record + queue.append + conditional queue.append + trackMessageSent). Atoms accept BUSINESS inputs only — Modal confirmations, Keyboard.dismiss, composer field reset, image-state clearing all stay in the component. Not to be confused with **Daemon** (Session's machine-side runtime form) — SessionController is App-side. Anything that is NOT bound to a specific Session — global settings, profiles, side-panel layout — stays OUT of SessionController. Pure ephemeral UI inside a SessionView (scroll position, focus, popover visibility) also stays out; the seam is for state that survives across mounts. `setPromptSuggestion` (1 call site, ephemeral UI hint) also stays out. Test surface is three protocol invariants pinned by the seam: (1) queue-paused ↔ retry-dispatcher coupling, (2) manual-send ↔ AutoOptionSend turn-dedup reset (cross-domain), (3) per-session subscription cleanup on unmount. A previously-considered fourth invariant — "permissionMode-change cascading side effects" — is explicitly NOT in scope because current code has no cascade; introducing it would invent behavior rather than capture existing seams.
_Avoid_: SessionRuntime (collides with Daemon's "runtime form" semantics), SessionViewModel (no MVVM precedent elsewhere in the App), SessionState (too generic — collides with storage's many `session*State` shapes), SessionStore (collides with the Zustand store namespace).
...
- **Session deactivation vs archival** — two distinct seams. `sessionDeactivate(userId, sessionId)` (ADR-0060) is presence-only: flip inactive + invalidate heartbeat cache + emit `session-activity(false)`; used by the Supervisor run/fix completion flows, which own any kill signal separately. `sessionArchive(ctx, sessionId)` additionally sends `session-terminate` to the daemon and runs inside a Context/inTx flow. Do not merge them.
...
- **Session** stays 1:1 with a **live** Claude Code native session — but Claude Code's `--resume <claudeId>` opens a brand-new native session whose first turn is preconditioned on an earlier session's transcript. When the Account triggers Resume from the App, the daemon spawns a fresh Happy Session whose **SessionMessages originate in another Session's Claude transcript** (CLI-side one-shot replay into the new Session, deterministic localId-deduped). The new Session's `metadata.claudeSessionId` still points at exactly one live Claude session id; the historical content is owned by the new Session for all purposes (cost, dedup, fork anchors). "Resume" is the Claude Code feature name, not a Happy first-class entity; "transcript replay" is the CLI mechanism, not a domain term.
