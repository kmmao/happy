...
Happy Coder is a mobile/web client system for remotely controlling Codex and Codex with end-to-end encryption. Users run `happy` instead of `Codex` on their computer, then control sessions from phone/web.
...
|---------|------|---------|-------------|
| **happy-cli** | `packages/happy-cli` | CLI wrapper for Codex/Codex with daemon | `@kmmao/happy-coder` on npm |
...
System: `Mobile/Web ←→ Server (Fastify+Socket.IO) ←→ CLI Daemon ←→ Codex/Codex` with PostgreSQL+Redis+S3. All content E2E encrypted (AES-256-GCM / NaCl secretbox). Data isolation: `~/.happy` (stable) vs `~/.happy-dev` (dev).
...
### CLI-Specific Rules
- All debugging through file logs (never console output that disturbs Codex sessions)
