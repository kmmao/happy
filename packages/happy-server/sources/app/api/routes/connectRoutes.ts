...
export function connectRoutes(app: Fastify) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      // OAuth 完成后跳回的前端地址，仅从环境变量读取（不写死）
      const appBaseUrl = (
        process.env.APP_URL ||
        process.env.HAPPY_APP_URL ||
        ""
      ).replace(/\/$/, "");
      if (!appBaseUrl) {
        log(
          { module: "github-oauth", level: "error" },
          "APP_URL or HAPPY_APP_URL must be set for GitHub OAuth callback redirect",
        );
        return reply.code(500).send({
          error: "OAuth redirect not configured: set APP_URL or HAPPY_APP_URL",
        });
      }
      log(
        { module: "github-oauth" },
        `OAuth callback redirect base: ${appBaseUrl} (APP_URL=${process.env.APP_URL ? "set" : "unset"} HAPPY_APP_URL=${process.env.HAPPY_APP_URL ? "set" : "unset"})`,
      );

      const tokenData = await auth.verifyGithubToken(state);
      if (!tokenData) {
        log({ module: "github-oauth" }, `Invalid state token: ${state}`);
        return reply.redirect(`${appBaseUrl}?error=invalid_state`);
      }
      ...
      if (!clientId || !clientSecret) {
        return reply.redirect(`${appBaseUrl}?error=server_config`);
      }
      ...
      try {
        ...
        if (tokenResponseData.error) {
          return reply.redirect(
            `${appBaseUrl}?error=${encodeURIComponent(tokenResponseData.error)}`,
          );
        }
        ...
        if (!userResponse.ok) {
          return reply.redirect(`${appBaseUrl}?error=github_user_fetch_failed`);
        }
        ...
        return reply.redirect(
          `${appBaseUrl}?github=connected&user=${encodeURIComponent(userData.login)}`,
        );
      } catch (error) {
        log(
          { module: "github-oauth" },
          `Error in GitHub GET callback: ${error}`,
        );
        return reply.redirect(`${appBaseUrl}?error=server_error`);
      }
    },
  );
  ...
  app.delete(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return reply
          .code(500)
          .send({ error: "Failed to disconnect GitHub account" });
      }
    },
  );
