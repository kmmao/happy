      ...
      await receiveUsageLock.inLock(async () => {
        try {
          ...
          try {
            ...
            // Create a new usage report for each API call
            const report = await db.usageReport.create({
              data: {
                accountId: userId,
            ...
            log(
              ...
              `Failed to save usage report: ${error}`,
            );
            if (callback) {
              callback({
                success: false,
                error: "Failed to save usage report",
              });
            }
          }
        } catch (error) {
          log(
            { module: "websocket", level: "error" },
            `Error in usage-report handler: ${error}`,
          );
          if (callback) {
