...
describe("parseWebhookPRMerge", () => {
    ...
    describe("GitLab", () => {
        ...
        it("should return null for non-merged state", () => {
            ...
            expect(parseWebhookPRMerge("gitlab", body, "Merge Request Hook")).toBeNull();
        });
    ...
    describe("extractLinkedIssueNumbers (via GitHub PR)", () => {
        ...
        it("should be case-insensitive", () => {
            ...
            expect(result?.linkedIssueNumbers).toEqual(expect.arrayContaining([1, 2, 3]));
            expect(result?.linkedIssueNumbers).toHaveLength(3);
        });

        it("should extract multiple issues from body", () => {
            const result = parseGitHubPR("Fixes #10, closes #20\nResolves #30", "main");
            expect(result?.linkedIssueNumbers).toEqual(expect.arrayContaining([10, 20, 30]));
            expect(result?.linkedIssueNumbers).toHaveLength(3);
        });
        ...
        it("should return empty when no keywords match", () => {
            const result = parseGitHubPR("This PR adds a new feature for #42", "main");
        ...
        it("should collect issues from both body and branch", () => {
            ...
            expect(result?.linkedIssueNumbers).toEqual(expect.arrayContaining([10, 20]));
...
describe("getEventTypeHeader", () => {
    it("should extract GitHub event type", () => {
        expect(getEventTypeHeader("github", { "x-github-event": "pull_request" })).toBe("pull_request");
    });

    it("should extract Gitea event type", () => {
        expect(getEventTypeHeader("gitea", { "x-gitea-event": "issues" })).toBe("issues");
    });

    it("should extract GitLab event type", () => {
        expect(getEventTypeHeader("gitlab", { "x-gitlab-event": "Issue Hook" })).toBe("Issue Hook");
    });
...
describe("getDeliveryId", () => {
    it("should extract GitHub delivery ID", () => {
        expect(getDeliveryId("github", { "x-github-delivery": "abc-123" })).toBe("abc-123");
    });

    it("should extract Gitea delivery ID", () => {
        expect(getDeliveryId("gitea", { "x-gitea-delivery": "def-456" })).toBe("def-456");
    });

    it("should extract GitLab event UUID", () => {
        expect(getDeliveryId("gitlab", { "x-gitlab-event-uuid": "ghi-789" })).toBe("ghi-789");
    });
