...
function patch() {
    if (!fs.existsSync(SDK_CLI_PATH)) {
        // SDK >= 0.2.119: cli.js no longer ships in the main package. The
        // query adapter's ENABLE_TOOL_SEARCH=auto:100 env setting suppresses
        // deferred-tool behavior at runtime instead. Silent no-op.
        return;
    }
