...
export class CodexMcpClient {
  ...
  async connect(): Promise<void> {
    ...
    logger.debug(
      `[CodexMCP] Connecting to Codex MCP server using command: codex ${mcpCommand}`,
    );
    ...
    if (this.sandboxConfig?.enabled) {
      if (process.platform === "win32") {
        logger.warn(
          "[CodexMCP] Sandbox is not supported on Windows; continuing without sandbox.",
        );
      } else {
        try {
    ...
    try {
      ...
      if (this.sandboxCleanup) {
        try {
          ...
          logger.warn(
            "[CodexMCP] Failed to reset sandbox after connection error.",
            cleanupError,
          );
        } finally {
          this.sandboxCleanup = null;
  ...
  private registerPermissionHandlers(): void {
    ...
    this.client.setRequestHandler(ElicitRequestSchema, async (request) => {
      ...
      if (!this.permissionHandler) {
        logger.debug(
          "[CodexMCP] No permission handler set, denying by default",
        );
        return {
  ...
  private updateIdentifiersFromEvent(event: any): void {
    ...
    for (const candidate of candidates) {
      ...
      if (sessionId) {
        ...
        logger.debug(
          "[CodexMCP] Session ID extracted from event:",
          this.sessionId,
        );
      }
      ...
      if (conversationId) {
        ...
        logger.debug(
          "[CodexMCP] Conversation ID extracted from event:",
          this.conversationId,
        );
      }
    }
  ...
  private extractIdentifiers(response: any): void {
    ...
    if (meta.conversationId) {
      ...
      logger.debug(
        "[CodexMCP] Conversation ID extracted:",
        this.conversationId,
      );
    ...
    if (Array.isArray(content)) {
      for (const item of content) {
        if (!this.sessionId && item?.sessionId) {
          ...
          logger.debug(
            "[CodexMCP] Session ID extracted from content:",
            this.sessionId,
          );
        }
        if (
          ...
          logger.debug(
            "[CodexMCP] Conversation ID extracted from content:",
            this.conversationId,
          );
        }
      }
  ...
  clearSession(): void {
    ...
    logger.debug(
      "[CodexMCP] Session cleared, previous sessionId:",
      previousSessionId,
    );
  }
  ...
  storeSessionForResume(): string | null {
    logger.debug(
      "[CodexMCP] Storing session for potential resume:",
      this.sessionId,
    );
    return this.sessionId;
  }
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        "[CodexMCP] Error closing client, attempting transport close directly",
        e,
      );
      try {
    ...
    if (this.sandboxCleanup) {
      try {
        ...
        logger.warn(
          "[CodexMCP] Failed to reset sandbox during disconnect.",
          error,
        );
      } finally {
        this.sandboxCleanup = null;
    ...
    logger.debug(
      `[CodexMCP] Disconnected; session ${this.sessionId ?? "none"} preserved`,
    );
  }
}
