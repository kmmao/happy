...
export const claudeCliPath = resolve(
  join(projectPath(), "scripts", "claude_local_launcher.cjs"),
);
...
export async function claudeLocal(opts: {
  ...
  /** Model to pass to Claude Code via --model flag */
  model?: string;
}) {
      ...
      if (index !== -1) {
        if (withValue && index + 1 < opts.claudeArgs.length) {
          ...
          if (!nextArg.startsWith("-")) {
            ...
            opts.claudeArgs = opts.claudeArgs.filter(
  ...
  if (sessionIdFlag.found && sessionIdFlag.value) {
    ...
    logger.debug(
      `[ClaudeLocal] Using explicit --session-id: ${sessionIdFlag.value}`,
    );
  }
  ...
  if (!startFrom && !sessionIdFlag.value) {
    ...
    if (resumeFlag.found) {
      if (resumeFlag.value) {
        ...
        logger.debug(
          `[ClaudeLocal] Using provided session ID from --resume: ${startFrom}`,
        );
      } else {
        ...
        if (lastSession) {
          ...
          logger.debug(
            `[ClaudeLocal] --resume: Found last session: ${lastSession}`,
          );
        }
      }
    }
  ...
  if (!startFrom && !sessionIdFlag.value) {
    ...
    if (continueFlag.found) {
      ...
      if (lastSession) {
        startFrom = lastSession;
        logger.debug(
          `[ClaudeLocal] --continue: Found last session: ${lastSession}`,
        );
      }
    }
  }
  ...
  if (!opts.hookSettingsPath) {
    ...
    if (startFrom) {
      ...
      logger.debug(
        `[ClaudeLocal] Using explicit session ID: ${explicitSessionId}`,
      );
    ...
    if (startFrom) {
      ...
      logger.debug(
        `[ClaudeLocal] Fresh start, session ID will be provided by hook`,
      );
    }
  }
  ...
  try {
    ...
    await new Promise<void>((r, reject) => {
      ...
      if (!opts.hookSettingsPath) {
        ...
        const hasResumeFlag =
          opts.claudeArgs?.includes("--resume") ||
          opts.claudeArgs?.includes("-r");
        if (startFrom) {
      ...
      // Pass model to Claude Code via --model flag
      const model = opts.model || process.env.ANTHROPIC_MODEL;
      if (model) {
        args.push("--model", model);
        logger.debug(`[ClaudeLocal] Using model: ${model}`);
      }
      ...
      if (opts.hookSettingsPath) {
        ...
        logger.debug(
          `[ClaudeLocal] Using hook settings: ${opts.hookSettingsPath}`,
        );
      }
          ...
          rl.on("line", (line) => {
            try {
              ...
              switch (message.type) {
                ...
                default:
                  logger.debug(
                    `[ClaudeLocal] Unknown message type: ${message.type}`,
                  );
              }
            } catch (e) {
