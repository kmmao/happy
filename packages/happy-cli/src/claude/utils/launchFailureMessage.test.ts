...
describe('launchFailureMessage', () => {
  ...
  describe('with an Error it appends the message', () => {
    it('surfaces a native-binary launch failure that is otherwise invisible', () => {
      const error = new Error(
        'Native CLI binary for darwin-arm64 not found. Reinstall without --omit=optional, or set the executable path.'
      );
      expect(launchFailureMessage(error)).toBe(
        'Process exited unexpectedly: Native CLI binary for darwin-arm64 not found. Reinstall without --omit=optional, or set the executable path.'
      );
    });
