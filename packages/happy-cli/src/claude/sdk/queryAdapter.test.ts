...
describe("queryAdapter", () => {
  describe("mapOptions", () => {
    ...
    it("should map maxBudgetUsd", () => {
      const opts: QueryOptions = { maxBudgetUsd: 5.0 };
      const result = mapOptions(opts);

      expect(result.maxBudgetUsd).toBe(5.0);
    });

    it("should map maxBudgetUsd when zero", () => {
      const opts: QueryOptions = { maxBudgetUsd: 0 };
      const result = mapOptions(opts);

      expect(result.maxBudgetUsd).toBe(0);
    });

    it("should not set maxBudgetUsd when undefined", () => {
      const opts: QueryOptions = {};
      const result = mapOptions(opts);
    ...
    it("should not set thinking when undefined", () => {
      const opts: QueryOptions = {};
      const result = mapOptions(opts);
    ...
    it("should map effort levels", () => {
      const levels = ["low", "medium", "high", "max"] as const;
      for (const level of levels) {
        const opts: QueryOptions = { effort: level };
        const result = mapOptions(opts);
    ...
    it("should not set effort when undefined", () => {
      const opts: QueryOptions = {};
      const result = mapOptions(opts);
    ...
    it("should map promptSuggestions", () => {
      const opts: QueryOptions = { promptSuggestions: true };
      const result = mapOptions(opts);

      expect(result.promptSuggestions).toBe(true);
    });

    it("should not set promptSuggestions when false", () => {
      const opts: QueryOptions = { promptSuggestions: false };
      const result = mapOptions(opts);

      expect(result.promptSuggestions).toBeUndefined();
    });

    it("should map all SDK features together", () => {
      const opts: QueryOptions = {
        model: "claude-opus-4-6",
        effort: "high",
