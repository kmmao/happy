...
export function query(config: {
  ...
  const isJsFile =
    ...
    pathToClaudeCodeExecutable.endsWith(".cjs");
  ...
  // Strip CLAUDECODE so Claude Code doesn't refuse to start as a "nested session"
  // when Happy CLI is running inside a Claude Code session (e.g. during development)
