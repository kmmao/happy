  ...
  const mockSession = {
    ...
    rpcHandlerManager: {
      registerHandler: vi.fn(
        (name: string, handler: (params: any) => Promise<any> | any) => {
...
vi.mock("./AcpBackend", () => ({
  AcpBackend: class MockAcpBackend {
    ...
    offMessage(handler: (message: any) => void) {
      mocks.backendState.listeners = mocks.backendState.listeners.filter(
        (item) => item !== handler,
      );
    }
...
describe("runAcp", () => {
  ...
  const stripLogPrefix = (line: string) =>
    stripAnsi(line).replace(/^\[\d{2}:\d{2}\] /, "");
  const consoleLines = () =>
    mocks.mockConsoleLog.mock.calls
  ...
  it("wires backend messages through mapper into session envelopes", async () => {
    ...
    const envelopeTypes =
      mocks.mockSession.sendSessionProtocolMessage.mock.calls.map(
        ([envelope]) => envelope.ev.t,
      );
  ...
  it("emits thinking messages in default mode", async () => {
    ...
    expect(consoleLines()).toEqual(
      expect.arrayContaining(['Thinking: "Analyzing request"']),
    );
  });
  ...
  it("emits raw backend and envelope logs when verbose is enabled", async () => {
    ...
    expect(
      lines.some((line) =>
        line.startsWith("Outgoing raw backend message from opencode: "),
      ),
    ).toBe(true);
    expect(
      lines.some((line) =>
        line.startsWith("Incoming raw envelope for opencode: "),
      ),
    ).toBe(true);
  ...
  it("updates session metadata with ACP config options (models and operating modes)", async () => {
    ...
    const metadataHandlers = mocks.mockSession.updateMetadata.mock.calls.map(
      (call) => call[0],
    );
    const baseMetadata = {
      path: "/repo",
    ...
    const appliedMetadata = metadataHandlers.map((handler) =>
      handler(baseMetadata),
    );
