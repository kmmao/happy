...
function formatTextForConsole(text: string): string {
  return JSON.stringify(
    truncateForConsole(toSingleLine(text), ACP_EVENT_PREVIEW_CHARS),
  );
}
...
function formatAcpMessageForFrontend(
  agentName: string,
  msg: AgentMessage,
  detailed: boolean,
): AcpFormattedLog | null {
  switch (msg.type) {
    case "status":
      return null;
    case "model-output": {
...
function formatEnvelopeForServerLog(
  agentName: string,
...
function resolveRequestedCode(
  ...
  for (const option of options) {
    if (
      normalizeComparable(option.code) === normalizedRequested ||
      normalizeComparable(option.value) === normalizedRequested
    ) {
      return option.code;
    }
  }
...
function resolveRequestedLegacyModeCode(
  ...
  for (const mode of modes.availableModes) {
    if (
      normalizeComparable(mode.id) === normalizedRequested ||
      normalizeComparable(mode.name) === normalizedRequested
    ) {
      return mode.id;
    }
  }
...
function resolveRequestedLegacyModelCode(
  ...
  for (const model of models.availableModels) {
    if (
      normalizeComparable(model.modelId) === normalizedRequested ||
      normalizeComparable(model.name) === normalizedRequested
    ) {
      return model.modelId;
    }
  }
...
class GenericAcpPermissionHandler
...
export async function runAcp(opts: {
  ...
  const messageQueue = new MessageQueue2<AcpSwitchMode>((mode) =>
    hashObject(mode),
  );
  ...
  const waitForTurnEnd = () =>
    new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `Timed out waiting for ${opts.agentName} to finish the turn`,
          ),
        );
      }, TURN_TIMEOUT_MS);
  ...
  const stopRunnerFromBackendStatus = (
  ...
  const sendEnvelopes = (envelopes: SessionEnvelope[]) => {
    for (const envelope of envelopes) {
      ...
      if (verbose) {
        logAcp(
          "muted",
          `Incoming raw envelope for ${opts.agentName}: ${formatUnknownForConsole(envelope, ACP_RAW_PREVIEW_CHARS)}`,
        );
      }
    }
  };

  const switchPermissionModeIfRequested = async (
    ...
    if (modeSelector) {
      const resolved = resolveRequestedCode(
        modeSelector.options,
        requestedMode,
      );
      if (!resolved) {
        logger.debug(
          `[${opts.agentName}] Ignoring unknown ACP permission mode request: ${requestedMode}`,
        );
        return;
      }
      ...
      const switched = await backend.setSessionConfigOption(
        modeSelector.configId,
        resolved,
      );
      if (switched) {
    ...
    const resolvedLegacyMode = resolveRequestedLegacyModeCode(
      legacyModes,
      requestedMode,
    );
    if (!resolvedLegacyMode) {
      logger.debug(
        `[${opts.agentName}] Ignoring unknown ACP legacy mode request: ${requestedMode}`,
      );
      return;
    }
  ...
  const switchModelIfRequested = async (
    ...
    if (modelSelector) {
      ...
      if (!resolved) {
        logger.debug(
          `[${opts.agentName}] Ignoring unknown ACP model request: ${requestedModel}`,
        );
        return;
      }
    ...
    const resolvedLegacyModel = resolveRequestedLegacyModelCode(
      legacyModels,
      requestedModel,
    );
    if (!resolvedLegacyModel) {
      logger.debug(
        `[${opts.agentName}] Ignoring unknown ACP legacy model request: ${requestedModel}`,
      );
      return;
    }
  ...
  const onBackendMessage = (msg: AgentMessage) => {
    if (verbose) {
      logAcp(
        "muted",
        `Outgoing raw backend message from ${opts.agentName}: ${formatUnknownForConsole(msg, ACP_RAW_PREVIEW_CHARS)}`,
      );
    }
    ...
    if (msg.type === "event" && msg.name === "available_commands") {
      ...
      if (verbose) {
        logAcp(
          "muted",
          `Outgoing slash commands from ${opts.agentName} (${commands.length}):`,
        );
        for (const command of commands) {
          logAcp(
            "muted",
            `  /${command.name}${formatOptionalDetail(command.description, 160)}`,
          );
        }
      }
    ...
    if (msg.type === "event" && msg.name === "config_options_update") {
      ...
      if (configOptions) {
        if (verbose) {
          logAcp(
            "muted",
            `Outgoing config options from ${opts.agentName} (${configOptions.length}):`,
          );
          for (const option of configOptions) {
            if (option.type === "select") {
              ...
              logAcp(
                "muted",
                `  config=${option.id} category=${option.category ?? "unknown"} current=${option.currentValue} options=${optionValues.length}`,
              );
            } else {
              logAcp(
                "muted",
                `  config=${option.id} type=${option.type} category=${option.category ?? "unknown"}`,
              );
            }
          }
        }
        ...
        if (verbose) {
          if (modeSelector) {
            ...
            logAcp(
              "muted",
              `Outgoing mode options from ${opts.agentName} (${modeSelector.options.length}), current=${modeSelector.currentCode}:`,
            );
            ...
            logAcp(
              "muted",
              `Outgoing mode options from ${opts.agentName}: not reported in config options`,
            );
          }
          if (modelSelector) {
            sawModels = true;
            logAcp(
              "muted",
              `Outgoing model options from ${opts.agentName} (${modelSelector.options.length}), current=${modelSelector.currentCode}:`,
            );
            ...
            logAcp(
              "muted",
              `Outgoing model options from ${opts.agentName}: not reported in config options`,
            );
          }
        }
        session.updateMetadata((currentMetadata) =>
    ...
    if (msg.type === "event" && msg.name === "modes_update") {
      ...
      if (modes) {
        ...
        if (verbose) {
          logAcp(
            "muted",
            `Outgoing modes from ${opts.agentName} (${modes.availableModes.length}), current=${modes.currentModeId}:`,
          );
          for (const mode of modes.availableModes) {
            logAcp(
              "muted",
              `  mode=${mode.id} name=${mode.name}${formatOptionalDetail(mode.description ?? undefined, 160)}`,
            );
          }
        }
        session.updateMetadata((currentMetadata) =>
    ...
    if (msg.type === "event" && msg.name === "models_update") {
      ...
      if (models) {
        ...
        if (verbose) {
          logAcp(
            "muted",
            `Outgoing models from ${opts.agentName} (${models.availableModels.length}), current=${models.currentModelId}:`,
          );
          for (const model of models.availableModels) {
    ...
    const frontendMessage = formatAcpMessageForFrontend(
      opts.agentName,
      msg,
      verbose,
    );
  ...
  session.onUserMessage((message) => {
    ...
    if (typeof message.meta?.permissionMode === "string") {
      ...
      logger.debug(
        `[${opts.agentName}] Requested ACP permission mode: ${currentPermissionMode}`,
      );
    }

    if (
      message.meta &&
      Object.prototype.hasOwnProperty.call(message.meta, "model")
    ) {
      currentModel = message.meta.model ?? null;
      logger.debug(
        `[${opts.agentName}] Requested ACP model: ${currentModel ?? "null"}`,
      );
    }
  ...
  try {
    ...
    if (verbose) {
      if (!sawSlashCommands) {
        logAcp(
          "muted",
          `Outgoing slash commands from ${opts.agentName}: not reported yet`,
        );
      }
      if (!sawModes) {
        logAcp(
          "muted",
          `Outgoing modes from ${opts.agentName}: not reported yet`,
        );
      }
      if (!sawModels) {
        logAcp(
          "muted",
          `Outgoing models from ${opts.agentName}: not reported yet`,
        );
      }
    }
    ...
    while (!shouldExit) {
      ...
      try {
        if (
          typeof batch.mode.permissionMode === "string" &&
          batch.mode.permissionMode.length > 0
        ) {
          await switchPermissionModeIfRequested(batch.mode.permissionMode);
        }
        if (
          typeof batch.mode.model === "string" &&
          batch.mode.model.length > 0
        ) {
        ...
        logAcp(
          "error",
          `Prompt error from ${opts.agentName}: ${error instanceof Error ? error.message : String(error)}`,
        );
        clearPendingTurn(
    ...
    try {
      ...
      logger.debug(
        `[${opts.agentName}] Failed to reset permission handler:`,
        error,
      );
    }
    ...
    try {
      ...
      logger.debug(
        `[${opts.agentName}] Failed to stop Happy MCP server:`,
        error,
      );
    }
