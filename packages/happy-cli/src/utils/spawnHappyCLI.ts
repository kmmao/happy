...
export function spawnHappyCLI(
  ...
  // thinking it's a nested session. This happens when Happy CLI is
  // running inside a Claude Code session (e.g. during development/release).
  ...
  // Also strip CLAUDECODE from any explicitly-provided env
  if (spawnOptions.env && "CLAUDECODE" in spawnOptions.env) {
    delete (spawnOptions.env as Record<string, string | undefined>).CLAUDECODE;
  }

  const runtime = isBun() ? "bun" : "node";
  return spawn(runtime, nodeArgs, spawnOptions);
}
