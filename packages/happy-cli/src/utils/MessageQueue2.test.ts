...
describe("MessageQueue2", () => {
  ...
  it("should throw when pushing to closed queue", () => {
    ...
    expect(() => queue.push("message", "local")).toThrow(
  ...
  it("should batch messages with enhanced mode hashing", async () => {
    ...
    }); // Different disallowed tools

    // First batch - same permission mode and model
    const result1 = await queue.waitForMessagesAndGetAsString();
    expect(result1).not.toBeNull();
    expect(result1?.message).toBe("message1\nmessage2");
    expect(result1?.mode).toEqual({
      permissionMode: "default",
      model: "sonnet",
    });
  ...
  describe("tryTakeForMidTurn", () => {
    ...
    it("should take message with same cold hash", () => {
      const queue = new MessageQueue2<{ type: string; cold?: string }>(
        (mode) => `${mode.type}-${mode.cold ?? ""}`,
      );
  ...
  describe("waitForNewMessage", () => {
    it("should resolve immediately when queue has messages", async () => {
      const queue = new MessageQueue2<string>((mode) => mode);
      queue.push("existing", "A");

      const result = await queue.waitForNewMessage();
      expect(result).toBe(true);
      expect(queue.size()).toBe(1); // not consumed
    });

    it("should wait and resolve when message arrives", async () => {
      const queue = new MessageQueue2<string>((mode) => mode);

      const waitPromise = queue.waitForNewMessage();

      setTimeout(() => {
        queue.push("delayed", "A");
      }, 10);
    ...
    it("should resolve false when aborted", async () => {
      ...
      const waitPromise = queue.waitForNewMessage(ac.signal);
    ...
    it("should resolve false immediately when signal already aborted", async () => {
      const queue = new MessageQueue2<string>((mode) => mode);
      const ac = new AbortController();
      ac.abort();

      const result = await queue.waitForNewMessage(ac.signal);
      expect(result).toBe(false);
    });

    it("should resolve false when queue closes", async () => {
      const queue = new MessageQueue2<string>((mode) => mode);
    ...
    it("should support multiple concurrent waiters", async () => {
      ...
      const wait1 = queue.waitForNewMessage();
      const wait2 = queue.waitForNewMessage();

      setTimeout(() => queue.push("msg", "A"), 10);
