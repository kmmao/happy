  ...
  try {
    ...
    const spawnSession = async (
      ...
      try {
        ...
        // Layers 2–5 (profile resolution, startup script, daemon-fallback
        // merge + expansion, auth validation) live behind one deep seam so the
        // precedence rules are auditable and testable without driving a spawn.
        // Layer 1 (authEnv) stays here because the caller owns its cleanup
        // (the Codex home overlay). See assembleSessionEnvironment.ts.
        const envResult = await assembleSessionEnvironment({
          options,
          runtimeProfile,
          directory,
          happySessionId,
        ...
        const { finalSessionEnv, sessionScopedEnv, spawnId } = envResult;
