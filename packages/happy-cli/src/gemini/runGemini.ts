...
import { classifyGeminiError } from "@/gemini/utils/geminiErrorClassify";
      ...
      try {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            // Classification (quota vs retryable-empty vs …) lives in the pure
            // classifyGeminiError seam; this loop only owns the retry control flow.
            const cls = classifyGeminiError(promptError);

            // Quota is NOT retryable — surface it and bail out of the loop.
            if (cls.category === "quota") {
              messageBuffer.addMessage(cls.userMessage, "status");
              session.sendAgentMessage("gemini", {
                type: "message",
            ...
            if (cls.isRetryable && attempt < MAX_RETRIES) {
              logger.debug(
                `[gemini] Retryable error on attempt ${attempt}/${MAX_RETRIES}`,
              );
              messageBuffer.addMessage(
        ...
        if (isAbortError) {
          ...
          // Full failure taxonomy (404 / empty / rate-limit / quota / auth /
          // cli-missing / fallback) lives in the pure classifyGeminiError seam.
