 ...
 * `@kmmao/happy-wire/rpcDispatch` so both `@kmmao/happy-coder` and this
 * package reuse one contract — see the wire file for the rationale (one
 * of the "two adapters = real seam" deepenings). The local re-export
 * keeps existing import sites (`from './types'`) working unchanged.
 *
...
/**
 * Constructor config — `cipher` is the package-local `Cipher` (AES-GCM)
 * which structurally satisfies the wire's `RpcCipher`.
 */
