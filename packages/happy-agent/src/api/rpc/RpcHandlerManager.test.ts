 ...
 * `Cipher` (AES-256-GCM). A regression in that wiring would silently
 * produce wire envelopes the server can't decrypt — the wire-level
 * routing test would still pass and the bug would only surface in
 * production.
 */
