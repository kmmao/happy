/**
 * UPnP/NAT-PMP tunnel provider for happy-agent.
 * Uses miniupnpc CLI (upnpc) for port mapping.
 */
...
const DEFAULT_LEASE_SECONDS = 7200;
...
export class UpnpProvider implements TunnelProvider {
  ...
  async add(params: TunnelAddParams): Promise<TunnelOpResult> {
    ...
    const result = await this.execUpnpc([
      "-a", localIp, String(localPort), String(externalPort), proto, String(DEFAULT_LEASE_SECONDS),
    ]);
    if (!result.success) return result;
    return result.stdout?.includes("is redirected to")
      ? { success: true }
      : { success: false, error: result.stdout ?? "Unknown error" };
  }
  ...
  async remove(params: TunnelRemoveParams): Promise<TunnelOpResult> {
    ...
    return (tcpResult.success || udpResult.success)
      ? { success: true }
      : { success: false, error: tcpResult.error ?? udpResult.error ?? "Failed to remove" };
  }
  ...
  async renewLeases(): Promise<void> {
    ...
    for (const entry of info.entries) {
      ...
      await this.execUpnpc([
        "-a", localIp, String(entry.localPort), String(entry.remotePort), entry.protocol, String(DEFAULT_LEASE_SECONDS),
      ]);
...
function parseExternalIp(output: string): string | undefined {
  return output.match(/ExternalIPAddress\s*=\s*([\d.]+)/)?.[1];
}

interface UpnpMapping {
  protocol: string; externalPort: number; internalIp: string;
  internalPort: number; description: string; leaseTime: number;
}
...
function parseMappingList(output: string): UpnpMapping[] {
  ...
  while ((match = regex.exec(output)) !== null) {
    mappings.push({
      protocol: match[1], externalPort: parseInt(match[2], 10),
      internalIp: match[3], internalPort: parseInt(match[4], 10),
      description: match[5], leaseTime: parseInt(match[6], 10),
    });
  }
  return mappings;
}
