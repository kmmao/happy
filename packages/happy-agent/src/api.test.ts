...
function makeRawSessionWithDataKey(
  ...
  const encryptedAgentState = agentState
    ? encryptWithDataKey(agentState, sessionKey)
    : null;
  ...
  const raw: RawSession = {
    ...
    ...("dataEncryptionKey" in overrides
      ? { dataEncryptionKey: overrides.dataEncryptionKey }
      : {}),
  };
...
function makeRawSessionLegacy(
  ...
  const encryptedAgentState = agentState
    ? encryptLegacy(agentState, creds.secret)
    : null;
...
describe("api", () => {
  ...
  describe("resolveSessionEncryption", () => {
    it("resolves dataKey variant when dataEncryptionKey is present", () => {
      const { raw, sessionKey } = makeRawSessionWithDataKey(creds, {
        name: "test",
      });
  ...
  describe("listSessions", () => {
    ...
    it("handles mixed dataKey and legacy sessions", async () => {
      ...
      const legacySession = makeRawSessionLegacy(creds, { name: "old" }, null, {
        id: "sess-old",
      });
    ...
    it("throws on 401 with re-authenticate message", async () => {
      ...
      const err = new (AxiosError as any)("Unauthorized", {
        response: { status: 401 },
      });
    ...
    it("throws on 404", async () => {
      const { AxiosError } = await import("axios");
      const err = new (AxiosError as any)("Not Found", {
        response: { status: 404 },
      });
      mockedAxios.get.mockRejectedValueOnce(err);
    ...
    it("throws on 500 server error", async () => {
      const { AxiosError } = await import("axios");
      const err = new (AxiosError as any)("Internal Server Error", {
        response: { status: 500 },
      });
      mockedAxios.get.mockRejectedValueOnce(err);
  ...
  describe("createSession", () => {
    ...
    it("throws on server error during create", async () => {
      const { AxiosError } = await import("axios");
      const err = new (AxiosError as any)("Server Error", {
        response: { status: 500 },
      });
  ...
  describe("deleteSession", () => {
    ...
    it("throws on 404", async () => {
      ...
      await expect(deleteSession(config, creds, "bad-id")).rejects.toThrow(
        "Not found",
      );
    });
