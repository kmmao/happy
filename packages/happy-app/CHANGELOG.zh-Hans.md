...
### 会话列表
- 新增：临时会话标签页重新支持按项目分组。会话归到项目分组头下（项目路径 + 机器名），分组按最近活动时间排序，组内会话同样按最近活动排序。当多个项目的会话交错时，你现在能直接看出每条属于哪个项目，不用再逐行去看路径。
- 改进：分组内的行不再重复显示已冗余的行内项目路径 —— 路径只在分组头出现一次，不再挂在每一行上。终端标题、Worktree 分支箭头、守护进程标记仍会显示，因为这些是分组头没有的每会话独有信息。

### Agent 循环
- 新增：通用的、服务端托管的循环，长按菜单里加了「立即执行」。立刻触发一轮迭代，让你无需等下一个调度节拍就能确认循环配置是否可用。
- 新增：创建循环表单加了「每轮全新会话」开关。默认跨迭代复用同一个 guardian 会话（上下文累积，改了 prompt 会叠加在旧上下文上）；新模式下每轮 spawn 全新会话，编辑后的 prompt 立刻干净生效，长期状态靠 memory.md 跨迭代传递。
- 新增：创建循环表单加了 AI backend 配置文件选择器。循环现在可以通过选择 profile 跑在非默认 provider（MiniMax、GLM 等）上；模型列表跟随所选 profile，且在创建 / 编辑 / adopt 三种模式下均能预填与回写。
- 修复：新建循环会话静默默认成 200K 上下文窗口。创建循环现在从「设置 → Agents」默认模型 seed（Claude Code 默认的 Opus 跑在 1M），CLI 也对从未指定模型的存量循环回退到 1M 默认值。模型选择器改为可折叠，并附「跟随设置默认」提示。
- 新增：循环编辑模式。工作流详情面板的「编辑」入口会重新打开创建循环表单，可原地修改循环的 prompt、调度、模型、effort、名称和 bootstrap 命令。

### Agent 目标栏
- 新增：为 PTY 模式会话加了权威的 Agent 目标栏。`/goal` 状态直接从 Claude transcript 中扫描，并渲染成 composer 上方的目标栏，带清除和编辑操作 —— 让 App 反映出与原生会话正在追踪的同一个目标。

### 修复
- 修复：一条已完整送达的助手回复在 App 里冻结在开头几个字符，而 TUI 显示的是完整内容。非流式消息（以及列表滚动时 cell 被重新挂载的消息）现在会立刻完整渲染；只有挂载之后新增的文本才走打字机动画。在后台 agent 场景下最明显。
- 修复：Yolo/bypass 会在计划选择器渲染前就自动批准 ExitPlanMode，导致「Clear context & execute」选项恰好在长上下文 429 的关键路径上够不着。ExitPlanMode 现在始终会走到选择器；真正的无人值守流程改用 CLI 的 `HAPPY_YOLO_EXIT_PLAN_AUTO_APPROVE=1`。

### 底层
- 在 CLI、Server、App 三端深化了可测试 seam（ADR-0056 至 ADR-0073），并补充了 RPC 安全、同步逻辑、agent 目标状态可见性等测试。调用点无行为变化。
...
### 图片 + 文字发送
- 修复发送图片连同文字时产生两条多余「[Request interrupted by user]」提示的问题。prompt 投递的回显校验把多行消息(文字 + 单独一行的图片引用)误判为发送失败,触发了两次中断重试。

### 中断按钮
- 修复中断按钮把刚发送的内容残留在 Claude 输入框的问题。Claude Code 在中断时会把被取消的 prompt 恢复回输入框,导致下一条消息与残留内容粘连重发。现在每次发送前都会清空输入框。
...
### 上下文窗口控制
- 改进按钮标签从 "AUTO" 改成 "200K"，与对侧 "1M" 对称，消除"自动 compact"的歧义。
- 修复 8 种语言的 ON 状态 tooltip 把阈值写成 `(200K)` —— 那是窗口大小不是阈值。10 种语言统一改成 `(150K)`（真实的 75% 触发点）。
- 修复 10 种语言的 OFF 状态 tooltip 都说"切回去时会在 75% 自动压缩" —— happy 现在只发提示，不自动压缩。改为"切到 200K + 75% 提示"（每种语言本地化）。

## 2.42.0 - 2026-06-18

Caveman 模式开启时发送 `/clear`，App 现在会先弹窗询问"清空后是否保持 Caveman？" —— 之前 `/clear` 会把对话上下文（`/caveman` 的激活就在里面）一起清掉，结果用户被静默打回普通输出模式，从 App 这边看不到任何提示。

### Caveman skill 切换
- 新增确认弹窗：Caveman 开启时发送 `/clear` 会先弹窗。"保持启用"会在 `/clear` 后自动追加一条 `/caveman` 重新激活；"仅清空"则只清空上下文。两种选择都会真正发出 `/clear`，弹窗只决定是否重新启用 Caveman。
- 改进发送时序：复用现有 `pendingQueueDispatcher` 的串行机制，`/caveman` 排在 `/clear` 后面入队，等 Claude 重新 idle 才发出，所以重新启用一定落在新上下文上，零额外协调代码。

## 2.41.0 - 2026-06-17

输入框上方的 Caveman skill 徽章现在始终可见 —— 之前只有在 Caveman 已激活时才会显示，从 App 这边无法看到 off→on 的入口，需要手动输入 `/caveman`。徽章现在常驻显示，从任何设备都能点一下开关。

### Caveman skill 切换
- 新增常驻 Caveman 徽章，紧邻连接状态：未激活时显示为空心胶囊（`textSecondary` 描边），激活时填充 `theme.colors.success`（绿色），一眼能看出当前模式。
- 改进点击行为为双向：未激活时点击直接发送 `/caveman`（启用无副作用，跳过确认）；已激活时仍弹出原确认弹窗后发送 `stop caveman`。

## 2.40.2 - 2026-06-17

### 上下文窗口控制
- 修复输入框展开时 AUTO/1M 切换 chip 消失的问题。展开状态用的是另一个进度组件(`ContextProgressBar`),2.40.1 没覆盖到,现在也接入了同一个切换,折叠 / 展开两个状态都能看到。

## 2.40.1 - 2026-06-17

### 上下文窗口控制
- 修复 AUTO/1M 切换 chip 在用量未达 90% 时不显示的问题。之前 chip 嵌在「低上下文警告」分支里,所以新会话上根本看不到。现在只要会话支持 autoCompact,chip 就始终显示;百分比文字仍然按老规则(剩余 ≤ 10% 或在设置里打开了「始终显示上下文用量」)显示。

## 2.40.0 - 2026-06-17

Standalone「转为定时」现在强制选择项目后才能保存 —— 之前留空会静默给 server 发 `projectId=null`,server 端兜底成字面字符串 `~`,导致 daemon 在自己 cwd 下创建一个名叫 `~` 的目录(通常落在 `$HOME/~/`),用户的定时任务全跑在那里。现在 cron 触发的会话落到正确的目录。

### 定时任务
- 新增 standalone「转为定时」弹窗的项目选择 —— 按所选机器级联展示该机器上的项目,未选项目时按钮禁用,确保 cron 运行时不会落到错误目录。
- 修复 server 端对历史数据 / 第三方脚本写入的、`projectId` 为空的定时任务的兜底:把字面 `"~"` 改成 `os.homedir()`,即使仍有这种行进入,也不再生成 `./~/` 目录。

## 2.39.0 - 2026-06-17

会话头部上下文用量芯片旁新增 AUTO/1M 切换开关。AUTO 模式（默认）让 Claude 留在 200K 窗口，并在用量到 75% 时由 happy 自动 `/compact`；切到 1M 则启用 1M 大上下文，由 Claude TUI 自己管理压缩。切换是会话级的，下一条用户消息生效，并走经过优化的冷重启路径 —— 即使在大会话里也能平滑过渡。

### 上下文窗口控制
- 新增上下文用量芯片旁的 AUTO/1M 切换。AUTO（默认）让 Claude 留在 200K 窗口，用量到 150K（75%）自动跑 `/compact`，避免触底。点击切到 1M 大上下文模式 —— happy 不再自动压缩，由 TUI 自己在约 80% 时处理。
- 改进切换 toggle（以及任何模型/模式切换）时的冷重启路径：新 PTY 启动后增加 5s 宽限期，不再把 Claude TUI 的 `--resume` 历史回放误判为真正的首次响应。结果是卡住的会话在 45–90s 而非 138s 时被检测到，并自动重投原始消息。

### 稳定性
- 修复 plan 模式 `ExitPlanMode` 或 `/deploy-server` 这类 slash command 后会触发的「No response for 138s」一类卡顿。丢失的延续 prompt 现在会自动重投，而不再静默消失。

## 2.38.0 - 2026-06-12

Picker 提醒现在能在 Web 端触达你：当 Claude 的终端阻塞在编号选择器或发出通知信号时，Web App 会弹出浏览器原生通知；所有平台的会话列表也会直接高亮被阻塞的会话行。

### Picker 与通知提醒
- 新增 Web 端浏览器原生通知 —— 终端 picker 提示和通知信号会触发系统通知，切换标签页也不会错过需要输入的会话。
- 新增会话行高亮 —— 终端阻塞在 picker 时，会话列表内对应行会在所有平台上高亮提示。

### 稳定性
- 改进聊天渲染管线 —— 消息可见性过滤、重复 diff 清理和工具调用分组现在走同一个经过完整测试的共享模块。
- 改进数据同步层 —— 服务端状态更新与本地 UI 变更严格分离，杜绝一类状态一致性问题。

## 2.37.0 - 2026-06-12

会话副标题现在可以实时显示 Claude 正在做什么。PTY 模式会话会把 TUI 状态面（spinner 动词、耗时、token 计数、进度）实时同步到 App，无需打开 Raw Terminal 就能一眼看到「Reasoning… · 12s · 1.2k tokens」—— 覆盖主会话列表、最近会话、机器详情页和聊天头部四处入口。

### 实时会话状态
- 新增运行中会话的实时 TUI 状态行 —— Claude 终端里的 spinner 动词、耗时秒数、输出 token 计数和进度百分比，会在回合运行期间替代副标题里的静态路径/标题，回合结束后自动回落。
- 新增「等待选择」通知 —— 当 Claude 终端被编号选择框（picker）阻塞等待键盘输入时，App 会推送横幅并附上捕获的选项文本，提示你去 Raw Terminal 作答。
- 新增 ConEmu 风格进度上报（OSC 9;4）：hook/构建产生的确定性进度按会话跟踪，并以百分比形式显示在实时状态行中。
- 新增聊天头部的实时状态行 —— 运行中时副标题从「进程 ID · cwd」切换为实时活动行。

### 会话面板
- 修复移动端会话面板中 Claude 标签页不渲染的问题。

### 模型
- 新增 Claude Fable 5（claude-fable-5）的端到端支持。

### 稳定性
- 重构同步更新摄取管线为单一接缝（ADR-0026），实时消息处理更可预测。

## 2.36.8 - 2026-06-09

Footprint 面板的小幅可读性修复：操作占比图例里的长工具名之前会被省略号截断（如 `mcp__codegraph__c...`、`mcp__happy__chang...`），导致用户无法分辨某个色块到底对应哪个 MCP 工具。

### Footprint 面板
- 修复操作占比图例中长工具名被截断的问题。移除了图例项上的 `numberOfLines={1}` + `maxWidth: 120` 约束；`mcp__codegraph__codegraph_files`、`mcp__happy__change_title` 这类长名称现在完整渲染。图例容器本身支持换行，较长的条目会自然流到下一行，而不是挤在一行里。

## 2.36.7 - 2026-06-09

补上了 2.36.5 复盘中遗留的最后一个 UX 缺口：当用户在预览浮层之外显式发出所预览的条目（chip ▶ 或头部「立即发送」按钮）时，队列预览浮层会停留在打开状态并展示过期快照。2.36.5 移除了「条目消失即自动关闭」的逻辑以避免自动派发时清掉正在进行的编辑 —— 但这也让显式用户操作同样无人处理。本次版本两头兼顾。

### 消息队列
- 修复 chip ▶ 按钮发送的正是用户正在预览的条目时，预览浮层不关闭的问题。chip ▶ 会把该条目排到队首并打断 AI 以便下一个发出，所以浮层现在也会随之关闭。对「其他」条目按 chip ▶ 不影响当前预览（你并没有发出正在预览的那条）。
- 修复队列横幅「立即发送」按钮在即将发出所预览条目时浮层不关闭的问题。该按钮发送队首条目；如果所预览条目恰好在队首，浮层会关闭。队列靠后的条目不会触发关闭。
- 保留 V6 对自动派发本身的「不自动关闭」行为 —— 自动派发移出条目时浮层仍保持打开（避免悄悄清掉进行中的编辑）。只有「发出当前条目」路径上的显式用户操作才会关闭浮层。

## 2.36.6 - 2026-06-09

补上了 2.36.5 复盘发现的「保存并发送」路径上的两个正确性缺口：与自动派发逻辑的多发竞态，以及条目已从队列移除后发送管线拒绝导致的静默丢消息路径。

### 消息队列
- 修复对某个队列条目执行「保存并发送」时可能把「下一个」队列条目也悄悄发出去的竞态。处理器现在会在打断运行中的会话或发送之前，先设置自动派发的 in-flight 门闩（并启动派发逻辑使用的同一个 15 秒保底计时器）—— 这样派发逻辑无法在打断确认窗口内插队发出下一条。
- 修复条目已从队列移除后底层发送管线拒绝导致的静默丢消息。现在只有在 `sync.sendMessage` 确认接受消息「之后」才会移除条目；拒绝或抛错时条目保留在队列中、释放派发门闩，并弹窗告知用户发送失败可以重试。
- 调整 `sync.sendMessage` 返回 `Promise<boolean>`（原为 `Promise<void>`）。`true` = 消息进入本地管线（队列或发件箱）；`false` = 无法推进（缺少加密/会话）。忽略返回值的既有调用方行为不变。

## 2.36.5 - 2026-06-08

清理了 2.36.3 代码复盘中暴露的队列消息编辑器的剩余边界情况。编辑现在可以完整往返文件附件而不丢失文件名提示，预览会立即重新渲染已保存内容而不是显示编辑前快照，「保存 / 保存并发送」在条目被自动派发抢走时也不再静默失败。

### 消息队列
- 修复保存成功后编辑浮层仍显示保存前内容的问题。父组件现在会用已保存的消息 + 图片数 + 展示文本刷新所预览条目，预览模式、再次进入编辑和取消按钮看到的都是最新状态。
- 修复条目已被自动派发后「保存并发送」被静默吞掉的问题。保存 / 保存并发送回调现在返回布尔值；返回 `false` 时浮层会显示「该消息已发送」横幅并保持打开，而不是无反馈地关闭。
- 修复队列暂停时「保存并发送」无任何动作的问题。现在会把编辑后的条目弹出队列直接发送（绕过暂停标志和自动派发逻辑），用户的显式意图优先于队列级暂停。
- 修复文件附件的文件名提示在编辑往返后被永久剥离的问题。`[image: /path | filename.pdf]` 之前会被解析为只剩 `/path` 再序列化；现在分段会携带可选文件名并原样输出。
- 修复编辑内容既无文本也无图片时「保存 / 保存并发送」按钮静默无操作的问题。空编辑时按钮现在置灰禁用，失败状态可见。
- 修复自动派发移走正在编辑的条目时浮层在编辑中途关闭的问题。「条目消失即自动关闭」逻辑已移除；浮层保持打开，用户对已不存在的条目保存时会显示过期错误。显式取消（chip ×）仍会关闭浮层。
- 修复内部状态残留（过期错误、编辑缓冲）在条目之间串扰的问题。打开不同的队列条目时浮层现在按 localId 干净地重新挂载。
- 修复 Web 端 chip 内部操作按钮（✏️、▶、×）同时触发预览浮层的问题。内部 Pressable 现在阻止事件冒泡，外层 chip 的点按不再连带触发。

## 2.36.4 - 2026-06-08

补上 2.36.3 之后仍存在的「两次快速发送仍会同时上线」缺口。输入框的 onSend（以及浮动选项 / 书签发送器）之前在 isRunning 为 false 时走直接调用 sync.sendMessage 的快捷路径，完全绕过队列。在服务器回推 `running=true` 之前的约 50 毫秒窗口内的两次点按都会走该快捷路径。现在所有发送都汇入队列，只有自动派发逻辑会调用 sync.sendMessage，该竞态从结构上不可能发生。

### 消息队列
- 修复所有用户发起的发送路径（输入栏、浮动选项、书签）改为始终入队，不再按 isRunning 分支。自动派发逻辑逐条发送；AI 空闲时可见的「已排队 chip」闪现至多一个渲染帧。
- 修复重复 localId 竞态 —— sync.sendMessage 内部的「会话运行中 → 重新入队」守卫会把刚移出的条目用相同 localId 推回队列，派发逻辑随后尝试发送重复项。自动派发现在传入仅本地的 `bypassRunningCheck` 标志，该守卫只对用户发起的发送生效。
- 修复自动派发的保底计时器在派发进行中被每次 effect 重跑静默清除的问题。3 秒计时器之前从 useEffect 返回，React 在下一次长度变化时就会清理它。现在计时器存在 ref 里跨重跑存活，只在 `isRunning` 真正升起（或组件卸载）时清除，并加长到 15 秒。

## 2.36.3 - 2026-06-08

打磨了 AI 忙碌时出现的消息队列栏：队列条目现在逐条发出（修复了打断后可能一次发出多条的竞态），每个条目都可以在发送前就地编辑，带图片的消息有清晰的相机角标和更整洁的预览面板。

### 消息队列
- 修复 AI 一空闲就可能连续发出两条以上队列消息的派发竞态。自动派发器现在会等 AI 真正进入运行状态后才释放下一条，并带 3 秒保底（离线/被拒时）。
- 新增队列横幅头部的「暂停队列 / 恢复」开关。暂停时即使 AI 空闲，队列条目也保持不动 —— 用户需要用 chip ▶ 或头部「立即发送」逐条释放。
- 新增队列条目的就地编辑。每个 chip 现在有 ✏️ 按钮，预览面板有编辑开关。消息发出前可以重写文本、移除单张图片；「保存」更新队列条目，「保存并发送」将其排到队首并触发打断。与自动派发竞争的编辑会显示友好的「已发送」提示。
- 改进队列消息的标注方式。含图片的条目现在同时显示小相机角标（多图带数量）和原始文本预览，而不是被「发送了一张图片」悄悄替换。纯图片消息仍使用合成标签，但以强调色渲染。
- 改进预览面板布局 —— 多图消息以两列网格渲染，文本分段带左侧强调条更易扫读，头部新增「N 张图片 · M 字符」快捷元信息。

## 2.36.2 - 2026-06-05

收紧了聊天头部布局 —— Claude 的实时工作目录从原先单独的第三行，合并到「进程 ID」副标题那一行，形成 `进程 ID 99118 · gs-frontend` 这样的单行展示。cwd 标签现在**始终显示**（即使 Claude 还没离开启动目录，也会显示启动目录末段名），让你随时一眼看到「自己在哪」，同时省下一行垂直空间。

### 聊天头部
- 改进聊天头部布局 —— 把实时工作目录折叠到「进程 ID」副标题那一行，形成 `进程 ID N · <cwd>` 的格式，移除了 2.36.0 引入的独立第三行。每个会话页节省一行垂直空间。
- 改进 cwd 展示逻辑 —— 只要路径数据可用就一定显示。Claude 还没离开启动目录时退化为启动目录末段名（如 `gs-frontend`），不再隐藏；Claude 已经移到子目录或别处时仍显示紧凑的相对形式（子目录 `./src/app`，旁支 `…/parent/name`）。
- 改进长副标题处理 —— `进程 ID · cwd` 在副标题单行内尾部截断；之前挂在聊天内容上方的 chip 组件已移除。

### 幕后改动
- 在已有的 `formatActiveCwd` 之外新增 `formatSessionCwdLabel` 辅助函数 —— 在 activeCwd / launchPath 之间挑选最合适的标签，并严格退化到 basename，调用方拿到的总是一个短小、可安全渲染的字符串。覆盖了 18 个单元测试（已有 10 个 + 新增 8 个），涵盖 POSIX / Windows / 旁支目录 / 路径缺失等场景。
- 移除 `SessionCwdBadge` 组件，以及 `ChatHeaderView` 的 `launchPath` / `activeCwd` props —— 头部现在严格保持两行（`title` + `subtitle`），cwd 组合逻辑迁移到副标题构建的同一处（`SessionView.tsx`）。

## 2.36.1 - 2026-06-03

修复了 Web 端「粘贴最新截图」的问题 —— 你在别的 App 里刚截完图、立刻切回浏览器按 Cmd+V 时，输入框有时仍会贴上一张旧截图，即便操作系统的剪贴板里其实已经是新图。

### 聊天输入
- 修复 Web 端粘贴时偶尔贴出上一张旧截图的问题。Chrome 在页面重新获得焦点后，对操作系统剪贴板的同步是异步完成的 —— 这导致 `e.clipboardData` 和粘贴回调里**同步**调用的 `navigator.clipboard.read()` 可能都返回上一张快照。现在粘贴处理器会先等一小段时间再读取剪贴板，并在第一次读到的图片大小与同步备份图完全相同时（这是剪贴板尚未同步完成的强信号）再延迟重试一次，确保最新那张截图能被正确粘入。
