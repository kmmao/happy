...
export const ru: TranslationStructure = {
  ...
  workflows: {
    ...
    sectionModel: "Модель",
    sectionEffort: "Усилие рассуждений",
    effortDefault: "По умолчанию",
    modelFollowsSettings: "Использует модель по умолчанию из настроек",
    loopEditModalTitle: "Изменить цикл",
    detailEdit: "Изменить",
    ...
    actionRunNow: "Запустить сейчас",
    actionRunNowSuccessTitle: "Цикл запущен",
    actionRunNowSuccessMessage: "Запущена новая итерация — сессии появятся ниже через мгновение.",
    actionRunNowErrorTitle: "Не удалось запустить цикл",
    ...
    loopBootstrapSlashCommandPresetsLabel: "Частые скиллы",
    loopBootstrapSlashCommandPresetNone: "Без",
    ...
    loopIssueHintToggleShow: "💡 О записи результатов в Git",
    loopIssueHintToggleHide: "Скрыть подсказку",
    webhookIssueHint: "Have the Agent run `happy issue comment <n> --body \"…\"` or `issue close <n>` before exiting — token is already in env.",
    webhookIssueHintToggleShow: "💡 О комментировании Issue",
    webhookIssueHintToggleHide: "Скрыть подсказку",
  },
  ...
  settings: {
    ...
    appLockSubtitle: "Требовать PIN-код или биометрию для открытия приложения",
  ...
  settingsFeatures: {
    ...
    scoringModelDescription: "Переопределить модель LLM для оценки релевантности вариантов по провайдеру. Оставьте пустым для значений по умолчанию (Anthropic → Haiku, OpenAI → GPT-4o-mini).",
    scoringModelProvider: "Модель {{provider}}",
    scoringModelPlaceholder: "По умолчанию",
  ...
  sessionInfo: {
    ...
    worktreeActivity: "Активность worktree",
    worktreeCreated: "Создан worktree",
    worktreeRemoved: "Удалён worktree",
    recentFileChanges: "Недавние изменения файлов",
    ...
    exportTranscript: "Экспортировать транскрипт",
    exportTranscriptSubtitle: "Копировать сообщения сессии как JSONL в буфер обмена",
    exportTranscriptSuccess: "Транскрипт скопирован в буфер обмена",
    exportTranscriptFailed: "Не удалось экспортировать транскрипт",
    exportTranscriptEmpty: "Нет сообщений для экспорта",
  },
  ...
  session: {
    ...
    pastedContent: "Вставленный контент",
    ...
    queuePausedWithCount: ({ n }: { n: number }) => `Пауза · ${n} сообщ. в очереди`,
    sendNow: "Отправить сейчас",
    pauseQueue: "Пауза очереди",
    resumeQueue: "Возобновить",
    queuedMessagePreview: "Предпросмотр сообщения",
    editQueuedMessage: "Редактировать сообщение",
    editMode: "Изменить",
    saveChanges: "Сохранить",
    saveAndSendNow: "Сохранить и отправить",
    cancelEdit: "Отмена",
    queueItemNotFound: "Это сообщение уже отправлено или удалено, сохранение невозможно.",
    saveAndSendFailed: "Не удалось отправить отредактированное сообщение. Оно осталось в очереди — можно повторить попытку.",
    imagesCount: ({ n }: { n: number }) => `${n} изобр.`,
    charsCount: ({ n }: { n: number }) => `${n} симв.`,
    ...
    codeExpandLines: ({ n }: { n: number }) => `Развернуть (${n} строк)`,
  ...
  machine: {
    ...
    agentLoopMaxUsdPerRun: "Макс. бюджет на запуск (USD)",
    agentLoopMaxUsdPerRunPlaceholder: "напр. 1.00",
    agentLoopMaxUsdPerRunInvalid: "Бюджет на запуск должен быть положительным числом",
    agentLoopMaxUsdPerDay: "Макс. дневной бюджет (USD)",
    agentLoopMaxUsdPerDayPlaceholder: "напр. 5.00",
    agentLoopMaxUsdPerDayInvalid: "Дневной бюджет должен быть положительным числом",
    agentLoopTotalCostUsd: "Общая стоимость",
    agentLoopTodayCostUsd: "Стоимость сегодня",
    agentLoopLastRunCostUsd: "Стоимость последнего запуска",
  ...
  tasks: {
    ...
    worktreeIsolation: "Изоляция Worktree",
    worktreeIsolationDesc: "Создаёт выделенный Git worktree для этого задания, чтобы текущий рабочий каталог остался нетронутым.",
    statusQueued: "Queued",
  ...
  triggers: {
    ...
    profileSection: "Профиль бэкенда AI",
  ...
  settingsAppLock: {
    ...
    enableSubtitle: "Требовать PIN-код для открытия приложения на этом устройстве",
    changePin: "Изменить PIN-код",
    biometric: "Разблокировка по биометрии",
    biometricSubtitle: "Используйте Face ID, Touch ID или отпечаток пальца для разблокировки",
    timeout: "Запрашивать PIN-код",
    timeoutSubtitle: "Выберите, через какое время в фоне приложение снова заблокируется.",
    timeoutImmediate: "Сразу",
    timeout30s: "Через 30 секунд",
    timeout1m: "Через 1 минуту",
    timeout5m: "Через 5 минут",
    timeoutNever: "Никогда",
    lockNow: "Заблокировать сейчас",
    footer: "Блокировка приложения управляет доступом к приложению только на этом устройстве. Она не влияет на сквозное шифрование вашего аккаунта.",
    setupTitle: "Задать PIN-код",
    setupEnterNew: "Введите новый PIN-код",
    setupConfirm: "Подтвердите PIN-код",
    setupHint: "Выберите 6-значный PIN-код",
    setupMismatch: "PIN-коды не совпадают. Попробуйте снова.",
    unlockTitle: "Введите PIN-код",
    unlockPrompt: "Введите PIN-код для разблокировки",
    incorrect: "Неверный PIN-код",
    lockedOut: ({ seconds }: { seconds: number }) => `Слишком много попыток. Повторите через ${seconds} с`,
    biometricPrompt: "Разблокировать Happy",
    biometricButton: "Использовать биометрию",
    forgotPin: "Забыли PIN-код?",
    forgotTitle: "Сбросить блокировку приложения?",
    forgotMessage: "Чтобы сбросить PIN-код, вы выйдете из аккаунта. Войдите снова, чтобы задать новый — ваши данные в безопасности.",
    forgotConfirm: "Выйти",
    disableTitle: "Отключить блокировку приложения?",
    disableMessage: "Для открытия приложения на этом устройстве больше не потребуется PIN-код или биометрия.",
  ...
  projects: {
    ...
    tabKnowledge: "Знания",
    tabConfig: "Настройки",
    tabTraces: "Трассировка",
    tracesEmpty: "Нет вызовов инструментов",
    tracesEmptySubtitle: "Вызовы инструментов появятся здесь во время активных сессий",
    tracesNoSession: "Нет активной сессии",
    ...
    knowledgeFilterSummary: "Сводка сеанса",
    knowledgeFilterRepoMap: "Карта репозитория",
    ...
    knowledgeConfigModeAutoHint: "До 5 записей, ранжированных по релевантности ключевых слов. Лучший баланс контекста и стоимости токенов",
    knowledgeConfigModeFullHint: "До 20 записей за сессию. Больше контекста, но выше расход токенов",
    knowledgeConfigModeMinimalHint: "Только профиль проекта (стек, соглашения). Отдельные записи не инжектируются",
    knowledgeConfigSensitivity: "Чувствительность сбора",
    knowledgeConfigSensitivityConservativeHint: "Извлечение каждые 5 ценных ходов или 60 мин. Меньше записей, но выше качество",
    knowledgeConfigSensitivityBalancedHint: "Извлечение каждые 3 ценных хода или 30 мин. Баланс охвата и шума",
    knowledgeConfigSensitivityAggressiveHint: "Извлечение после каждого ценного хода (макс. 10 мин). Больше записей, но больше шума",
    knowledgeConfigCollection: "Триггеры сбора",
    knowledgeConfigTrackFileEdits: "Отслеживать изменения файлов",
    knowledgeConfigTrackFileEditsHint: "Когда ИИ создаёт или изменяет файлы → отмечает этот ход как ценный. Каждые 3 хода отправляются на сервер, не при каждом файле",
    knowledgeConfigTrackTokens: "Отслеживать глубокий анализ",
    knowledgeConfigTrackTokensHint: "Когда ход без файловых изменений, но вывод >1500 токенов (глубокий анализ, архитектурное планирование) → тоже отмечается как ценный",
    knowledgeConfigSummary: "Сводка сессии",
    knowledgeConfigSummaryHint: "Когда сессия завершается с ≥2 ценными ходами → автоматически создаёт сводку. Дорабатывается ИИ",
    ...
    knowledgeConfigMergeHint: "Поиск записей с ≥75% сходством, объединение 2-5 в кластере (макс. 5 кластеров/запуск). Исходные записи становятся «заменёнными». Каждые 6 часов",
    ...
    ciStatus: "Статус CI",
    ciStatusEmpty: "Нет записей о запусках CI. Настройте webhook GitHub, чтобы видеть статус сборки.",
  },
  project: {
    segments: {
      board: "Доска",
  ...
  diagnostics: {
    ...
    smartCleanEmpty: "Устаревших сессий не обнаружено",
    smartCleanConfirmTitle: "Очистить устаревшие сессии",
    smartCleanConfirmMessage: ({ count }: { count: number }) =>
      `Завершить ${count} устаревш${count === 1 ? "ую" : "их"} сесси${count === 1 ? "ю" : "й"}?`,
    smartCleanSuccess: ({ killed }: { killed: number }) =>
      `Очищено ${killed} устаревш${killed === 1 ? "ая" : "их"} сесси${killed === 1 ? "я" : "й"}`,
    smartCleanFailed: "Не удалось очистить устаревшие сессии",
  ...
  supervisor: {
    ...
    dimPrReview: "Проверка кода PR",
    dimPrReviewNote: "Безопасность, качество и покрытие тестами изменений PR",
    ...
    autoLoopSection: "Авто-старт при ухудшении здоровья",
    autoLoopEnabled: "Включить авто-цикл",
    autoLoopDesc: "Автоматически запустить supervisor-цикл, когда анализ сообщает здоровье на/выше порога",
    autoLoopThreshold: "Порог здоровья",
    autoLoopThresholdHint: ({ value }: { value: number }) => `Авто-цикл срабатывает при healthScore ≥ ${value}. Больше — хуже здоровье.`,
    autoLoopDebounce: "Время остывания (часы)",
    autoLoopDebounceHint: ({ hours }: { hours: number }) => `Ждать ${hours}ч между авто-запусками для этого проекта`,
    autoLoopDebounceHintOff: "Без остывания — авто-цикл срабатывает при каждом подходящем завершении (тестовый режим)",
    autoLoopResetButton: "Сбросить остывание сейчас",
    autoLoopResetTitle: "Сбросить остывание авто-цикла?",
    ...
    loopConfigMaxFailures: "Макс. последовательных сбоев",
    loopConfigMaxFailuresHint: ({ count }: { count: number }) => `Цикл прекращается после ${count} сбоев подряд`,
    loopConfigHealthTarget: "Цель по здоровью",
    loopConfigHealthTargetHint: ({ value }: { value: number }) => `Выход, когда healthScore опустится до ${value} или ниже`,
    loopConfigHealthTargetHintOff: "Цель по здоровью не задана — цикл завершится через no_new_actions / goal_achieved / max_iterations / лимит стоимости",
    loopConfigDuration: "Лимит по времени",
    loopConfigDurationHint: ({ hours }: { hours: number }) => `Через ${hours}ч цикл принудительно завершается с причиной timeout`,
    loopConfigEmptyIters: "Подтверждение сходимости",
    loopConfigEmptyItersHint: ({ count }: { count: number }) => `Цикл выходит с «цель достигнута» после ${count} подряд пустых анализов`,
    loopConfigEmptyItersHintLegacy: "Цикл выходит после первого пустого анализа (legacy)",
  ...
  permissions: {
    sheetTitle: "Требуется разрешение",
  ...
  projectConfig: {
    sectionSupervisor: "Монитор состояния",
    sectionResearch: "Исследование конкурентов",
    sectionKnowledge: "База знаний",
    sectionNotifications: "Уведомления и профиль",
    sectionIntegrations: "Интеграции и вебхук",
    supervisorMode: "Режим",
    modeSuggest: "Предлагать",
    modeSemiAuto: "Полуавтоматический",
    modeAuto: "Автоматический",
    supervisorSchedule: "Расписание",
    supervisorDimensions: "Параметры",
    dimensionsEnabled: "включено",
    supervisorPush: "Push-триггер",
    researchDimensions: "Параметры",
    researchDirection: "Направление",
    researchRules: "Правила",
    rulesLines: "строк",
    researchHint: "Настройки исследования по умолчанию задаются здесь. Переопределения для каждого запуска — во вкладке Исследование.",
    knowledgeNotAvailable: "Конфигурация знаний недоступна. Сначала синхронизируйте проект с сервером.",
    notificationsEnabled: "Уведомления",
    defaultProfile: "Профиль AI",
    profileDefault: "По умолчанию",
    webhookEvents: "СОБЫТИЯ ВЕБХУКА",
    webhookNoEvents: "Пока нет событий вебхука",
    viewAll: "Показать все",
    machineTriggers: "Триггеры машины",
    movedToConfigHint: "Конфигурация перенесена во вкладку Настройки",
    sectionContextMd: "Контекст проекта",
    contextMdDesc: "Автоматически вставляется в каждую новую сессию агента",
    contextMdEdit: "Редактировать CONTEXT.md",
    contextMdNoSession: "Нет активной сессии — запустите сессию для редактирования",
    contextMdTitle: "Редактировать контекст проекта",
    contextMdSave: "Сохранить",
    contextMdSaved: "Сохранено",
    contextMdSaveError: "Не удалось сохранить",
    contextMdPlaceholder: "Опишите ваш проект: технологический стек, соглашения, ключевые архитектурные решения...",
    contextMdHint: "Хранится в .happy/CONTEXT.md в репозитории вашего проекта.",
  },
  suggestions: {
    refresh: "Обновить",
  },
  claudeControl: {
    cost: {
      ...
      loading: "Вычисление стоимости…",
      error: "Стоимость недоступна",
    },
    version: {
      remoteCli: "Удалённый Claude Code",
      happyCli: "Happy CLI",
      unknown: "Неизвестно",
    },
    color: {
    ...
    fileViewer: {
      ...
      close: "Закрыть",
      loading: "Загрузка…",
      truncated: "Обрезан до 1 МиБ",
      deniedBlacklistedPath: "Путь заблокирован политикой безопасности CLI",
      deniedPermissionDenied: "Доступ запрещён Claude Code",
      deniedTooLarge: "Файл слишком большой",
      deniedNotFound: "Файл не найден",
      deniedError: "Не удалось прочитать файл",
    },
    mcp: {
      title: "Вызвать инструмент MCP",
      stepSelect: "1. Выбрать инструмент",
      stepConfirm: "2. Подтвердить",
      reviewHeading: "Проверьте перед вызовом",
      confirmCheckbox: "Я понимаю, что это запустит инструмент MCP напрямую",
      invoke: "Вызвать",
      result: "Результат",
      errorNotWhitelisted: "Сервер MCP не в белом списке удалённого CLI",
      errorServerUnavailable: "Сервер MCP недоступен",
      errorToolNotFound: "Инструмент не найден на сервере",
      errorInvalidArguments: "Недопустимые аргументы",
      errorPermissionDenied: "Доступ запрещён",
      errorSdkNotImplemented: "Ожидание Claude Agent SDK — среда выполнения mcp_call ещё недоступна",
    ...
    contextUsage: {
      ...
      detailLoading: "Загрузка содержимого…",
      detailError: "Не удалось загрузить содержимое",
      detailItems: "{n} элементов",
  ...
  world: {
    title: "Мир",
    subtitle: "Войдите в глобальное рабочее пространство агентов",
    exitWorld: "Выйти из мира",
    streamMode: "Поток событий",
    noEvents: "Событий пока нет",
    emptyTitle: "Матрица тихая",
    emptyDescription: "Программы будут появляться здесь по мере работы. Запустите сессию или задание, чтобы оживить мир.",
    filterAll: "Все",
    definitionPanel: "Определение мира",
    narrative: "Нарратив",
    laws: "Законы",
    policy: "Политика",
    notSet: "Не задано",
    narrativePlaceholder: "Опишите текущее направление и цели...",
    chainMode: "Цепочки",
    agentMode: "Агенты",
    noAgents: "Нет активных агентов",
    agentModeEmptyDescription: "Агенты появятся здесь, когда сессии будут активны на подключённых машинах.",
    agentIdle: "Простой",
    noChainResults: "Цепочки не соответствуют поиску",
    policyDescDisabled: "Только вручную — никаких автоматических действий",
    policyDescSuggest: "Предлагает следующие шаги, ждёт подтверждения",
    policyDescSemiAuto: "Автоматически обрабатывает рутинные действия, эскалирует исключения",
    policyDescAuto: "Полностью автономный — действует по всем решениям без ожидания",
    inspector: "Детали события",
    ...
    actionRead: "Отметить прочитанным",
    actionDismiss: "Отклонить",
    actionRetry: "Повторить",
    actionDone: "Готово",
    eventProject: "Проект",
    openIn: "Открыть в",
    openKnowledge: "Открыть знания",
    actions: "Действия",
    densityMode: "Плотность",
    densitySystem: "Система",
    densityEvents: "событий",
    densityActive: "активно",
    densityViewStream: "Смотреть в потоке",
    densityEmpty: "Пока нет активности",
    densityEmptyDesc: "Проекты появятся здесь по мере событий.",
    densityTotal: "всего",
    densityRecent: "30 мин",
    densityLastActivity: "последняя активность",
    emptyFilterTitle: "Нет подходящих событий",
    emptyFilterDesc: "Никакие события не соответствуют текущему фильтру. Попробуйте очистить его.",
    emptyClearFilter: "Очистить фильтр",
    emptyActionNewSession: "Новая сессия",
    emptyActionQueueTask: "Добавить задание",
    emptyQuickStart: "Введите / для команд",
  },
} as const;
