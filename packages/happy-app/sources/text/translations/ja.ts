...
export const ja: TranslationStructure = {
  ...
  machine: {
    ...
    automation: "自動化",
    automationQueuedHint: "セッションに割り当てられるのを待っています。",
    automationRunningHint: "現在実行中、または今まさに割り当て中です。",
    automationGuardiansHint: "再利用や再アタッチに使える保存済みの継続セッションです。",
    ...
    automationLoopsTotalHint: "このマシンに現在設定されているすべての Loop 数です。",
    automationLoopsActiveHint: "トリガーやスケジュール発火時にまだ自動実行できる Loop です。",
    automationLoopsBlockedHint: "失敗、ポリシー、または手動の安全停止で止められている Loop です。",
    automationLoopsPausedHint: "現在無効化され、再開待ちの Loop です。",
    automationLoopsPendingEventsHint: "まだ処理されていないキュー済みイベントを保持している Loop です。",
    automationLoopsPolicyStoppedHint: "ポリシーまたは保護ロジックによって最近停止された Loop です。",
    automationOpenLoopsHint: "このマシンの Loop 設定、トリガー、実行状態を開きます。",
    automationTotalAuditEventsHint: "このマシンに保存されているローカル自動化監査記録の総数です。",
    automationGuardianReuseCountHint: "既存の Guardian セッションがゼロから開始されずに再利用された回数です。",
    automationGuardianReuseRateHint: "再利用可能な実行のうち、Guardian セッションの再利用に成功した割合です。",
    automationGuardianResetCountHint: "保存済み Guardian 継続性を消去して再開始する必要があった回数です。",
    automationSessionReattachedCountHint: "中断や再起動後にセッションへ再アタッチできた回数です。",
    automationWatchdogStopsHint: "安全のため watchdog がセッションを強制停止した回数です。",
