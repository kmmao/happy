...
export const en = {
  ...
  workflows: {
    ...
    // Per-kind empty state — surfaces when the user filters into a kind
    // chip that has zero rows yet. Each gets its own CTA wired straight
    // to the matching standalone create modal so the user doesn't have
    // to go hunting for the header "+" menu.
    ...
    // Adopt-session menu item + modal (Phase 2 sessionAdopt — bind an
    // existing Session to an existing automation Workflow).
    ...
    // Section A — fold-out flow preview shown below the prompt input.
    ...
    // Section B — secret reveal extras.
    ...
    // Cron preset chips — one tap rewrites the cron input.
  ...
  diagnostics: {
    title: "Diagnostics",
    viewAll: "Diagnostics",
    viewAllHint: "Happy process status & termination tools",
    ...
    cleanRunaway: "Terminate Orphans",
    cleanConfirmTitle: "Terminate Orphan Processes",
    cleanConfirmMessage: "Kill all orphaned daemon and session processes?",
    cleanSuccess: ({ killed }: { killed: number }) =>
      `Terminated ${killed} process${killed !== 1 ? "es" : ""}`,
    cleanFailed: "Failed to terminate processes",
    smartClean: "Terminate Stale",
    smartCleanEmpty: "No stale sessions need termination",
    smartCleanConfirmTitle: "Terminate Stale Sessions",
    smartCleanConfirmMessage: ({ count }: { count: number }) =>
      `Kill ${count} stale session${count !== 1 ? "s" : ""}?`,
    smartCleanSuccess: ({ killed }: { killed: number }) =>
      `Terminated ${killed} stale session${killed !== 1 ? "s" : ""}`,
    smartCleanFailed: "Failed to terminate stale sessions",
