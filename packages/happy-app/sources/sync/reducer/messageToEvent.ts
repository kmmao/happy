...
export function parseMessageAsEvent(msg: NormalizedMessage): AgentEvent | null {
  ...
  if (msg.role === "agent") {
    for (const content of msg.content) {
      ...
      // Only convert to event if the corresponding tool-result is present and successful.
      // If no result yet (streaming) or result is an error, let normal tool rendering handle it.
      if (
        content.type === "tool-call" &&
        content.name === "mcp__happy__change_title"
      ) {
        const title = content.input?.title;
        if (typeof title === "string") {
          const toolResult = msg.content.find(
            (c): c is Extract<typeof c, { type: "tool-result" }> =>
              c.type === "tool-result" && c.tool_use_id === content.id,
          );
          ...
          // No result yet or result is an error — skip event conversion
        }
      }
    }
  }
...
export function isRateLimitMessage(text: string): boolean {
  ...
  if (
    ...
    /retry(?:ing)? in \d+\s*(?:second|seconds|s|ms)\b/.test(lower)
  ) {
    return false;
  }
