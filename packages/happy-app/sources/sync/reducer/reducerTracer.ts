    ...
    if (message.role === "agent") {
      for (const content of message.content) {
        ...
        if (content.type === "tool-call" && content.name === "Task") {
          ...
          // Link subagent ID to this Task for sidechain matching
          if (
            content.input &&
            typeof content.input === "object" &&
            "_subagentId" in content.input &&
            typeof content.input._subagentId === "string"
          ) {
            state.toolCallToMessageId.set(
              content.input._subagentId,
              message.id,
            );
            // Flush any orphaned messages already buffered under this subagent ID
            const subagentOrphans = processOrphans(
              state,
              content.input._subagentId,
              message.id,
            );
