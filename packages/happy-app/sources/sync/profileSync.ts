...
class ProfileSyncService {
    ...
    private emitEvent(event: ProfileSyncEvent): void {
        this.eventListeners.forEach(listener => {
            try {
                ...
                log.log(`[ProfileSync] Event listener error: ${error}`);
            }
        });
    }
    ...
    public async syncCliToGui(): Promise<AIBackendProfile[]> {
        ...
        try {
            ...
            log.log(`[ProfileSync] Retrieved ${currentProfiles.length} profiles from Happy settings`);
    ...
    public async getActiveProfile(): Promise<AIBackendProfile | null> {
        try {
            ...
            if (activeProfile) {
                log.log(`[ProfileSync] Retrieved active profile ${activeProfile.name} from Happy settings`);
                return activeProfile;
            }

            return null;
        } catch (error) {
            log.log(`[ProfileSync] Failed to get active profile: ${error}`);
            return null;
        }
    }
    ...
    public async autoSyncIfNeeded(guiProfiles: AIBackendProfile[]): Promise<void> {
        ...
        if (timeSinceLastSync > AUTO_SYNC_INTERVAL) {
            try {
                ...
                log.log(`[ProfileSync] Auto-sync failed: ${error}`);
