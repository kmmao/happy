...
export class Encryption {
  ...
  private sessionDataKeyHex = new Map<string, string | null>();
   ...
   * If a session already exists but the key has changed (e.g. after reconnect),
   * the old encryptor and cached data are replaced with new ones.
   */
  async initializeSessions(
    sessions: Map<string, Uint8Array | null>,
  ): Promise<void> {
    for (const [sessionId, dataKey] of sessions) {
      // If already initialized, check whether the key has changed
      if (this.sessionEncryptions.has(sessionId)) {
        const keyHex = dataKey ? encodeHex(dataKey) : null;
        const prev = this.sessionDataKeyHex.get(sessionId) ?? null;
        if (keyHex === prev) {
      ...
      this.sessionDataKeyHex.set(
        sessionId,
        dataKey ? encodeHex(dataKey) : null,
      );
    }
  }
  ...
  removeSessionEncryption(sessionId: string): void {
    this.sessionEncryptions.delete(sessionId);
    this.sessionDataKeyHex.delete(sessionId);
