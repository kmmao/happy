...
class AutoOptionSendService {
    ...
    disposeSession(sessionId: string): void {
        ...
        const abortAndDelete = (controllers: Map<string, AbortController>) => {
        ...
        abortAndDelete(this.semanticControllers);
        abortAndDelete(this.passiveScoringControllers);
        abortAndDelete(this.generationControllers);
        abortAndDelete(this.autoGenerationControllers);

        this.states.delete(sessionId);
        this.listeners.delete(sessionId);
        this.uiContexts.delete(sessionId);
        this.semanticScores.delete(sessionId);
        this.lastSemanticScoredAt.delete(sessionId);
        this.semanticMeta.delete(sessionId);
        this.lastPassiveScoredAt.delete(sessionId);
        this.generatedOptions.delete(sessionId);
        this.lastGeneratedTurnId.delete(sessionId);
        this.lastAutoGeneratedTurnId.delete(sessionId);
        this.lastGeneratedAt.delete(sessionId);
        this.lastPassiveGeneratedAt.delete(sessionId);
    ...
    private async triggerOptionGeneration(sessionId: string, messages: Message[]): Promise<void> {
        ...
        const latestAgentMsg = messages.find((m) => m.kind === "agent-text" && !("isThinking" in m && m.isThinking));
        const turnId = latestAgentMsg?.id ?? null;
        const turnCached = this.lastAutoGeneratedTurnId.get(sessionId);
