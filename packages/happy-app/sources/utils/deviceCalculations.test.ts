...
describe("responsive utilities", () => {
  ...
  describe("integration scenarios", () => {
    ...
    it("should identify Galaxy Z Fold inner screen as tablet with minWidthPoints", () => {
      ...
      const deviceType = determineDeviceType({
        diagonalInches: dimensions.diagonalInches,
        platform: "android",
        minWidthPoints: Math.min(673, 884), // 673dp min width
      });
    ...
    it("should identify Galaxy Z Fold outer screen as phone", () => {
      // Galaxy Z Fold outer screen: ~374x832 dp
      const deviceType = determineDeviceType({
        diagonalInches: 5.7,
        platform: "android",
        minWidthPoints: Math.min(374, 832), // 374dp min width
      });

      expect(deviceType).toBe("phone");
    });

    it("should identify Pixel Fold inner screen as tablet", () => {
      // Pixel Fold inner screen: ~616x827 dp
      const deviceType = determineDeviceType({
        diagonalInches: 6.5,
        platform: "android",
        minWidthPoints: Math.min(616, 827), // 616dp min width
      });

      expect(deviceType).toBe("tablet");
    });

    it("should keep normal Android phone as phone with minWidthPoints", () => {
      // Typical Android phone: ~393x851 dp
      const deviceType = determineDeviceType({
        diagonalInches: 5.9,
        platform: "android",
        minWidthPoints: Math.min(393, 851), // 393dp min width
      });

      expect(deviceType).toBe("phone");
    });

    it("should handle foldable threshold boundary at 500dp", () => {
      const atThreshold = determineDeviceType({
        diagonalInches: 6.0,
        platform: "android",
        minWidthPoints: 500,
      });
      expect(atThreshold).toBe("tablet");

      const belowThreshold = determineDeviceType({
        diagonalInches: 6.0,
        platform: "android",
        minWidthPoints: 499,
      });
      expect(belowThreshold).toBe("phone");
    });

    it("should support custom foldableMinWidthThreshold", () => {
      const deviceType = determineDeviceType({
        diagonalInches: 6.0,
        platform: "android",
        minWidthPoints: 550,
        foldableMinWidthThreshold: 600,
      });

      expect(deviceType).toBe("phone"); // 550 < 600
    });

    it("should not apply foldable detection on iOS", () => {
      const deviceType = determineDeviceType({
        diagonalInches: 5.8,
        platform: "ios",
        isPad: false,
        minWidthPoints: 659, // Even with high minWidth, iOS ignores it
      });

      expect(deviceType).toBe("phone");
    });

    it("should apply foldable detection on web with wide viewport", () => {
      const deviceType = determineDeviceType({
        diagonalInches: 5.8,
        platform: "web",
        minWidthPoints: 659,
      ...
      expect(deviceType).toBe("tablet"); // Wide web viewport triggers tablet mode
    });

    it("should keep narrow web viewport as phone", () => {
      const deviceType = determineDeviceType({
        diagonalInches: 5.8,
        platform: "web",
        minWidthPoints: 400,
      ...
      expect(deviceType).toBe("phone"); // Narrow viewport stays phone
    });

    it("should be backward compatible without minWidthPoints", () => {
      // Galaxy Z Fold5 unfolded: 7.6" diagonal screen, no minWidthPoints
      const deviceType = determineDeviceType({
        diagonalInches: 7.6,
        platform: "android",
      });

      expect(deviceType).toBe("phone"); // Without minWidthPoints, falls back to diagonal
    });
  });
