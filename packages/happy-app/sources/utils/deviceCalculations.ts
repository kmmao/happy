...
export function determineDeviceType(params: {
  ...
  minWidthPoints?: number; // Minimum dimension in dp (for foldable detection)
  foldableMinWidthThreshold?: number; // Min width dp threshold for foldable inner screen, default 500
}): "phone" | "tablet" {
  const {
    diagonalInches,
    platform,
  ...
  // Wide screen detection: foldable inner screens or wide web viewports
  // Foldable inner screens have minWidth 580-690dp vs regular phones 360-430dp
  // Also applies to web (e.g. foldable phone browser or wide viewport)
  if (
    platform !== "ios" &&
    minWidthPoints !== undefined &&
    minWidthPoints >= foldableMinWidthThreshold
  ) {
    return "tablet";
  }
