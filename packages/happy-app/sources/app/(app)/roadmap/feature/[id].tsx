...
const FeatureDetail = React.memo(() => {
    ...
    const [status, setStatus] =
        React.useState<RoadmapFeatureStatus>("planned");
    const [moscow, setMoscow] =
        React.useState<RoadmapMoscow>("should_have");
                ...
                <ItemGroup title={t("roadmap.statusLabel")}>
                    {ROADMAP_FEATURE_STATUSES.map((s, idx) => (
                        <Item
                            key={s}
                            title={t(
                                ROADMAP_FEATURE_STATUS_LABELS[s],
                            )}
                            onPress={() => setStatus(s)}
                            selected={s === status}
                            showDivider={
                                idx !==
                                ROADMAP_FEATURE_STATUSES.length - 1
                            }
                        />
                    ))}
                </ItemGroup>
                ...
                <ItemGroup title={t("kanban.actionsLabel")}>
                    ...
                    {hasConvertedTask && (
                        <Item
                            ...
                            onPress={() =>
                                router.push(
                                    `/kanban/task/${feature.convertedTaskId}`,
                                )
                            }
                            showDivider
                        />
