...
export default function DeviceInfo() {
  ...
  const reactiveDeviceType = useDeviceType();
  const deviceType = getDeviceType();
  const headerHeight = useHeaderHeight();
  const isRunningOnMacCatalyst = isRunningOnMac();
  const minWidthPoints = Math.min(windowDims.width, windowDims.height);
  ...
  return (
    <>
      ...
      <ItemList>
        ...
        <ItemGroup title="Foldable Detection">
          <Item
            title="Reactive Device Type"
            detail={reactiveDeviceType === "tablet" ? "Tablet" : "Phone"}
          />
          <Item title="isTablet (hook)" detail={isTablet ? "Yes" : "No"} />
          <Item
            title="Min Width (dp)"
            detail={`${Math.round(minWidthPoints)} dp`}
          />
          <Item title="Foldable Threshold" detail="500 dp" />
          <Item
            title="Foldable Triggered"
            detail={
              minWidthPoints >= 500
                ? `Yes (${Math.round(minWidthPoints)} >= 500)`
                : `No (${Math.round(minWidthPoints)} < 500)`
            }
          />
          <Item
            title="Platform Check"
            detail={`${Platform.OS} ${Platform.OS === "android" ? "(active)" : "(inactive)"}`}
          />
          <Item
            title="Window (reactive)"
            detail={`${Math.round(windowDims.width)} x ${Math.round(windowDims.height)}`}
          />
        </ItemGroup>
