...
export function getLoopDetailMessage(loop: MachineAgentLoop): string {
    return [
        ...
        loop.maxUsdPerRun ? `${t("machine.agentLoopMaxUsdPerRun")}: $${loop.maxUsdPerRun.toFixed(2)}` : undefined,
        loop.maxUsdPerDay ? `${t("machine.agentLoopMaxUsdPerDay")}: $${loop.maxUsdPerDay.toFixed(2)}` : undefined,
        loop.lastRunCostUsd != null ? `${t("machine.agentLoopLastRunCostUsd")}: $${loop.lastRunCostUsd.toFixed(4)}` : undefined,
        loop.totalCostUsd != null ? `${t("machine.agentLoopTotalCostUsd")}: $${loop.totalCostUsd.toFixed(4)}` : undefined,
        loop.todayCostUsd != null ? `${t("machine.agentLoopTodayCostUsd")}: $${loop.todayCostUsd.toFixed(4)}` : undefined,
