...
function SupervisorActionsScreen() {
    ...
    React.useEffect(() => {
        ...
        async function loadStats() {
            try {
                ...
                const data = await fetchActionStats(credentials, id).catch(
                    () => null,
                );
                if (!cancelled && data) {
                    setStats(data);
    ...
    }, [id, activeTab, actions.length, total, loadingMore]);
    ...
    const handleRefresh = React.useCallback(async () => {
        try {
            ...
            const [actionsData, statsData] = await Promise.all([
                ...
                fetchActionStats(credentials, id).catch(() => null),
            ]);
    ...
    const handleUpdated = React.useCallback(async () => {
        // Refresh the list and stats after an action is updated
        try {
            const credentials = await TokenStorage.getCredentials();
            if (!credentials) return;
