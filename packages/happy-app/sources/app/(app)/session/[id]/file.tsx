    ...
    const loadFile = async () => {
      try {
        ...
        if (sessionPath && sessionId) {
          try {
            // When viewing a historical commit, diff against its parent
            // Otherwise, diff working tree changes
            const diffCommand = commitHash
              ? `git diff --no-ext-diff ${commitHash}^..${commitHash} -- "${filePath}"`
