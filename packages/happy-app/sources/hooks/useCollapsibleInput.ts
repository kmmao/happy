...
export function useCollapsibleInput(
    ...
    // Default: collapsed when session has messages, expanded when empty
    // If user previously expanded this session, start expanded
    const [collapsed, setCollapsed] = React.useState(
        isPersistedExpanded ? false : hasMessages,
    );

    // Sync collapsed state when messages load (hasMessages goes false → true)
    // Only if user hasn't manually expanded
    const prevHasMessages = React.useRef(hasMessages);
    React.useEffect(() => {
        if (!prevHasMessages.current && hasMessages && !isPersistedExpanded) {
            setCollapsed(true);
        }
        prevHasMessages.current = hasMessages;
    }, [hasMessages, isPersistedExpanded]);
    ...
    const collapse = React.useCallback(() => {
        // Don't collapse if STT is active or images are pending
        if (isSttListening || hasPendingImages) return;
        setCollapsed(true);
