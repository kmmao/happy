/**
 * Auto-polling hook for issue lists.
 *
 * Periodically refreshes issues while the component is mounted and the app
 * is in the foreground. Pauses when backgrounded; triggers an immediate
 * refresh on foreground resume (if cooldown has elapsed).
 *
 * Relies on issueStore's built-in FETCH_COOLDOWN (30 s) for deduplication,
 * so manual RefreshControl pulls and auto-polls never conflict.
 */
...
export function useIssuePolling(
  ...
  React.useEffect(() => {
    ...
    const refresh = () => {
      ...
      issueStore.getState().refreshAllIssues(keys as string[], sid, paths);
    };
