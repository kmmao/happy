...
export const ToolGroupView = React.memo<ToolGroupViewProps>((props) => {
    const { group, metadata, sessionId, expanded, onToggle, permissionModeKey, contentMaxWidth } = props;
    const { theme } = useUnistyles();
    const summary = React.useMemo(() => generateGroupSummary(group.messages), [group.messages]);

    return (
        <View style={styles.outerContainer}>
            <View style={[styles.innerContainer, { maxWidth: contentMaxWidth ?? layout.maxWidth }]}>
                <Pressable
                    onPress={onToggle}
                    style={({ pressed }) => [
                        styles.header,
