...
function formatRelativeTime(updatedAt: number | null, nowMs: number): string {
    // Threshold/clock-skew math lives in relativeTimeParts (pure, tested);
    // this only maps the parts onto i18n strings.
    const parts = relativeTimeParts(updatedAt, nowMs);
    switch (parts.kind) {
        case "empty": return "";
        case "just-now": return t("session.progressTimeJustNow");
        case "minutes": return t("session.progressTimeMinutes", { n: parts.n });
        case "hours": return t("session.progressTimeHours", { n: parts.n });
        case "days": return t("session.progressTimeDays", { n: parts.n });
    }
}
