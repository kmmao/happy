...
export const CloneRepoModal = React.memo(function CloneRepoModal({
    ...
    const selectedHostName = selectedHost?.host;
    const isGitea = selectedHost?.provider === "gitea";
    ...
    const filteredRepos = useMemo(() => {
        ...
        // Text search filter (client-side — for GitHub this is the only search;
        // for Gitea this is an additional local refinement on server results)
        ...
        // Sort: recent repos first, then original order
        const recentRank = new Map<string, number>();
        recentRemoteRepos.forEach((repo, index) => {
            if (repo.host === selectedHostName) {
                recentRank.set(repo.repoUrl, index);
            }
        });
        const orderMap = new Map(searchedRepos.map((repo, index) => [repo.cloneUrl, index]));
    ...
    }, [recentRemoteRepos, repoSearch, repos, selectedHostName, visibilityFilter]);

    // Load first page or search results
    const handleLoadRepos = useCallback(async (options?: {
        forceRefresh?: boolean;
        query?: string;
    }) => {
        ...
        setCurrentPage(1);
        try {
            const result = await onLoadRepos(selectedHost, {
                forceRefresh: options?.forceRefresh,
                page: 1,
                perPage: REPOS_PER_PAGE,
                // Only send query to server for Gitea (server-side search).
                // GitHub ignores query — search is done client-side.
                query: isGitea ? (query || undefined) : undefined,
            });
    ...
    }, [onLoadRepos, selectedHost, isGitea]);

    // Load next page (append) — uses ref for synchronous guard
    const handleLoadMore = useCallback(async () => {
        if (!selectedHost || loadingMoreRef.current || !hasMore) return;
    ...
    }, [selectedHost, hasMore, currentPage, onLoadRepos, isGitea]);

    // Debounced search — for Gitea triggers server search, for GitHub just updates local filter
    const handleSearchChange = useCallback((text: string) => {
        setRepoSearch(text);
        if (!isGitea) return; // GitHub: client-side filter only, no debounced request
        if (searchTimerRef.current) {
            clearTimeout(searchTimerRef.current);
        }
        searchTimerRef.current = setTimeout(() => {
            handleLoadRepos({ query: text.trim() });
        }, SEARCH_DEBOUNCE_MS);
    }, [handleLoadRepos, isGitea]);
    ...
    const renderRepoItem = useCallback(({ item: repo, index }: { item: RemoteGitRepoEntry; index: number }) => {
    ...
    }, [selectedRepoUrl, filteredRepos.length, theme, styles, recentRemoteRepos, selectedHostName]);

    const keyExtractor = useCallback(
        (repo: RemoteGitRepoEntry) => `${repo.fullName}:${repo.cloneUrl}`,
        [],
    );
    ...
    }, [loadingMore, styles.footerLoader, theme.colors.textLink]);
