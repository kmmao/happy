...
export const IdeationView = React.memo(() => {
  ...
  const counts = React.useMemo(() => ideaCountByStatus(allIdeas), [allIdeas]);
  ...
  const keyExtractor = React.useCallback((item: IdeationIdea) => item.id, []);
  ...
  if (allIdeas.length === 0) {
    return <IdeationEmptyState onAdd={handleAdd} />;
  }
